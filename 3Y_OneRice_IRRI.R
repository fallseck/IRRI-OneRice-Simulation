...
for (REP in 1:nReps) {
  ...
  cat("----------------------------------------------------\n")
  cat(paste0("RUNING OF THE BREEDING SCHEME ", scenarioName[3]," ... \n"))
  cat("----------------------------------------------------\n")
  ...
  for (j in 1:Cohort3Y) {
    ...
    if (j < 3){
      # 1st Stage of Yield Evaluation (Seed Amplification): Season 4 (Year 2)
    ...
    if (j < 2){
      # 1st Stage of Yield Evaluation (Estimation Set): Season 5 & 6 (Year 3)
  ...
  for (b in iBreed:nYears) {
    ...
    tmp$Scenario[b+1] = scenarioName[3]
    tmp$Kind[b+1] = "GS"
  ...
  saveRDS(list(PARAM=param, 
               ...
               alphaP=freqAlphaP,
               SNP=freqSnptmp), 
          paste0("RES_",scenarioName[3], "_IRRI-OneRice_",g,"_",m,".rds"))
