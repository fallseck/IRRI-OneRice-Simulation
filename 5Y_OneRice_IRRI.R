...
for (REP in 1:nReps) {
  ...
  cat("----------------------------------------------------\n")
  cat(paste0("RUNING OF THE BREEDING SCHEME ", scenarioName[2]," ... \n"))
  cat("----------------------------------------------------\n")
  ...
  for (j in 1:Cohort5Y) {
    ...
    if (j < 2){ 
      ...
      # Genomic Prediction
      GS = RRBLUP(pop = F8_EST)

      F7_EBV = setEBV(pop = F7_OYT,
                      solution = GS,
                      value = 'bv')
    }
  }#END Pipeline
 
  Mean$Scenario = scenarioName[2]
  Mean$Kind = "GS"
  ...
  for (b in iBreed:nYears) {
    ...
    newParents = selectInd(pop = F7_EBV,
                           nInd = nParents,
                           use = 'ebv')
    
    # 2nd Stage of Yield Evaluation on AYT: Season 11 & 12 (Year 6)
    F9_AYT = selectInd(pop = F7_EBV,
                       nInd = nAYT,
                       use = 'ebv')
    # Advancement
    F9_AYT = self(F9_AYT)
    ...
    # Stage 1 (OYT)
    Mean$meanG_Sg1[iCycle] =  meanG(F7_EBV)
    Mean$varG_Sg1[iCycle] = varG(F7_EBV)[1]
    Mean$Accu[iCycle] = cor(bv(F7_EBV),ebv(F7_EBV))
    Mean$Cycle[iCycle] = iCycle-1
    Herr$OYT[b] = varG(F7_EBV)[1]/varP(F7_EBV)[1]
    #Stage 1 (ESTs)
    ...
    tmp$Scenario[b+1] = scenarioName[2]
    tmp$Kind[b+1] = "GS"
  ...
  saveRDS(list(PARAM=param, 
               ...
               alphaP=freqAlphaP,
               SNP=freqSnptmp), 
          paste0("RES_",scenarioName[2], "_IRRI-OneRice_",g,"_",m,".rds"))
